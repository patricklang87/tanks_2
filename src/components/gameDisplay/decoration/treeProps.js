...
export const initiateTrees = (topography) => {
    const treeArray = [];
    let numTrees = Math.floor(Math.random() * 200);
    for (let i = 0; i < numTrees; i++) {
        const newTree = { point: [0, 0], size: 0 };
        ...
        const minY = getYForXInLine({
            point1: startPoint,
            point2: endPoint,
            currentX: treeX,
        }) -
            0.8 * newTree.size;
        const treeY = minY + Math.floor(Math.random() * (canvasConstants.height - minY));
        newTree.point = [treeX, treeY];
        treeArray.push(newTree);
    }
    return treeArray;
};
export const drawTrees = (ctx, customProps) => {
